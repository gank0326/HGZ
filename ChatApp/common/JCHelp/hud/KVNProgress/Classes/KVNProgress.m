...
static CGFloat const KVNAlertViewWidth = 230.0f;
	...
	if ([self isFullScreen]) {
		...
		if (KVNIpad) {
            ...
            if (self.status && self.status.length > 0) {
                ...
                CGSize stringSize = [self.status sizeWithFont:self.statusLabel.font constrainedToSize:constraintSize];
...
- (void)updateStatusConstraints
{
    ...
    if (self.configuration.circleSize == 0) {
        self.circleProgressViewToStatusLabelVerticalSpaceConstraint.constant = 0.0f;
    }
