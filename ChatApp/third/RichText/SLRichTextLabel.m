...
static dispatch_queue_t richtext_attributed_label_parse_queue;
static dispatch_queue_t get_richtext_attributed_label_parse_queue() {
	if (richtext_attributed_label_parse_queue == NULL) {
		richtext_attributed_label_parse_queue = dispatch_queue_create("com.richtext.parse_queue", 0);
	}
	return richtext_attributed_label_parse_queue;
}

@interface SLRichTextLabel () <UIGestureRecognizerDelegate>
{
	NSMutableArray *_attachments;
	NSMutableArray *_linkLocations;
	NSMutableArray *_customColors;
...
@property (nonatomic, strong)    SLRichTextLabelURL *touchedLink;
@property (nonatomic, assign)    BOOL linkDetected;
@end

@implementation SLRichTextLabel

- (id)initWithFrame:(CGRect)frame withTapEvent:(BOOL)canTap {
...
- (void)commonInit {
	...
	_customColors           = [[NSMutableArray alloc] init];
...
- (void)cleanAll {
	...
	[_customColors removeAllObjects];
...
- (void)setFont:(UIFont *)font {
	if (font && _font != font) {
		...
		[self resetFont];
		for (SLRichTextLabelAttachment *attachment in _attachments) {
...
- (NSAttributedString *)attributedStringForDraw {
	if (_attributedString) {
		...
		for (SLRichTextLabelColor *customColor in _customColors) {
			if (customColor.range.location + customColor.range.length > [_attributedString length]) {
				continue;
			}
			UIColor *drawLinkColor = customColor.color;
			[drawString setTextColor:drawLinkColor range:customColor.range];
		}

		for (SLRichTextLabelURL *url in _linkLocations) {
...
- (SLRichTextLabelURL *)urlForPoint:(CGPoint)point {
	...
	for (int i = 0; i < count; i++) {
		...
		if (CGRectContainsPoint(rect, point)) {
			...
			SLRichTextLabelURL *url = [self linkAtIndex:idx];
			if (url) {
				return url;
			}
		}
...
- (id)linkDataForPoint:(CGPoint)point {
	SLRichTextLabelURL *url = [self urlForPoint:point];
...
- (SLRichTextLabelURL *)linkAtIndex:(CFIndex)index {
	for (SLRichTextLabelURL *url in _linkLocations) {
...
- (void)appendAttachment:(SLRichTextLabelAttachment *)attachment {
	attachment.fontAscent                   = _fontAscent;
	attachment.fontDescent                  = _fontDescent;
...
- (void)appendImage:(UIImage *)image
	...
	[self appendImage:image
	          maxSize:maxSize
	           margin:margin
	        alignment:SLRichTextImageAlignmentBottom];
}
...
- (void)appendImage:(UIImage *)image
	...
	SLRichTextLabelAttachment *attachment = [SLRichTextLabelAttachment attachmentWith:image
	                                                                           margin:margin
	                                                                        alignment:alignment
	                                                                          maxSize:maxSize];
...
- (void)appendView:(UIView *)view
            margin:(UIEdgeInsets)margin
         alignment:(SLRichTextImageAlignment)alignment {
	SLRichTextLabelAttachment *attachment = [SLRichTextLabelAttachment attachmentWith:view
	                                                                           margin:margin
	                                                                        alignment:alignment
	                                                                          maxSize:CGSizeZero];
	[self appendAttachment:attachment];
...
- (void)addCustomLink:(id)linkData
	...
	SLRichTextLabelURL *url = [SLRichTextLabelURL urlWithLinkData:linkData
	                                                        range:range
	                                                        color:color];
...
- (void)setTextColor:(UIColor *)textColor range:(NSRange)range {
	SLRichTextLabelColor *co = [[SLRichTextLabelColor alloc] init];
	co.color = textColor;
	co.range = range;
	[_customColors addObject:co];
}
...
+ (void)setCustomDetectMethod:(SLRichTextLabelBlock)block {
	[SLRichTextLabelURL setCustomDetectMethod:block];
}
...
- (void)drawAttachments {
	...
	for (CFIndex i = 0; i < numberOfLines; i++) {
		...
		for (CFIndex k = 0; k < runCount; k++) {
			...
			SLRichTextLabelAttachment *attributedImage = (SLRichTextLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
#pragma mark - 点击事件处理
- (BOOL)onLabelClick:(id)linkData {
	if (linkData) {
		if (_delegate && [_delegate respondsToSelector:@selector(richTextAttributedLabel:clickedOnLink:)]) {
			[_delegate richTextAttributedLabel:self clickedOnLink:linkData];
		}
		else {
			NSURL *url = nil;
...
- (void)recomputeLinksIfNeeded {
	...
	BOOL sync = length <= SLRichTextMinAsyncDetectLinkLength;
...
- (void)computeLink:(NSString *)text
	...
	LinkBlock block = ^(NSArray *links)
	{
		...
		if ([links count]) {
			for (SLRichTextLabelURL *link in links) {
	...
	if (sync) {
		NSArray *links = [SLRichTextLabelURL detectLinks:text];
		block(links);
	}
	else {
		dispatch_sync(get_richtext_attributed_label_parse_queue(), ^{
		    NSArray *links = [SLRichTextLabelURL detectLinks:text];
...
- (void)addAutoDetectedLink:(SLRichTextLabelURL *)link {
...
- (void)handleSingleTapFrom:(UITapGestureRecognizer *)singleRecognizer {
	if (_autoDetectLinks && self.touchedLink) {
		if (singleRecognizer.state == UIGestureRecognizerStateEnded) {
			[self onLabelClick:self.touchedLink.linkData];
			[self clearTouchLink];
		}
	}
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
	[self performSelector:@selector(clearTouchLink) withObject:nil afterDelay:0.3];
	return YES;
}
