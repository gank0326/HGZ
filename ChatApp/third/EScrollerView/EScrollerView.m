...
//        float pageControlWidth=noteView.bounds.size.width;
...
{
    if (_posterTimer != nil && [_posterTimer isKindOfClass:[NSTimer class]])
    {
        [self stop];
    }
}
...
#pragma mark timer
- (void)autoReloadPoster
{
    currentPageIndex++;
    if (currentPageIndex==[imageArray count])
    {
        currentPageIndex = 1;
        [scrollView scrollRectToVisible:CGRectMake(viewSize.size.width, 0, viewSize.size.width, scrollView.frame.size.height) animated:NO];
        [scrollView scrollRectToVisible:CGRectMake(viewSize.size.width*2, 0, viewSize.size.width, scrollView.frame.size.height) animated:YES];
    }
    else
    {
        [scrollView scrollRectToVisible:CGRectMake(viewSize.size.width*currentPageIndex, 0, viewSize.size.width, scrollView.frame.size.height) animated:YES];
    }
}
...
-(void)start
{
    _posterTimer = [NSTimer scheduledTimerWithTimeInterval:5
                                                      target:self
                                                    selector:@selector(autoReloadPoster)
                                                    userInfo:nil
                                                     repeats:YES];
}
